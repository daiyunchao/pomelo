...
var encodeBody = function(server, route, msgBody) {
  ...
  } else if (msgBody && msgBody.type ==="Buffer" && Array.isArray(msgBody.data)) {
    // 判断是否本来就是Buffer类型,如果是的话就不需要转换成字符串了
    msgBody = new Buffer(msgBody.data);
   } else {
